# Final Hiring Decision — Automation Implementation Guide
...
| Component | Responsibility in Final Decision |
|-----------|--------------------------------|
...
| **SharePoint** | Primary document library for `08_Final_Decision/` |
...
```
1. Open 09_Prompt_Library/ and 08_Final_Decision/Prompt.md in Cursor.
...
```python
# Pseudocode — production worker for WF-08
def process_item(workflow_run_id, payload):
    ...
    validated = validate_schema(result, schema="WF-08_output")
    if validated.confidence < 0.85:
        create_review_task(workflow_run_id, validated)
    else:
        save_artifact(f"08_Final_Decision/", validated)
...
### Inbound Webhook
```
POST /api/v1/workflows/08_Final_Decision/run
...
POST {subscriber_url}
{
  ...
  "artifact_path": "08_Final_Decision/REQ-2026-0142_CAN-88421_output.json"
}
```
...
- **Library:** `RecruitmentOps/08_Final_Decision/`
...
Parse inbound applications via dedicated alias `careers+08_Final_Decision@{{org.email_domain}}`.
