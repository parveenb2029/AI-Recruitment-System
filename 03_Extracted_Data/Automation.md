# Resume Data Extraction — Automation Implementation Guide
...
| Component | Responsibility in Resume Extractor |
|-----------|--------------------------------|
...
| **SharePoint** | Primary document library for `03_Extracted_Data/` |
...
```
1. Open 09_Prompt_Library/ and 03_Extracted_Data/Prompt.md in Cursor.
...
```python
# Pseudocode — production worker for WF-03
def process_item(workflow_run_id, payload):
    ...
    validated = validate_schema(result, schema="WF-03_output")
    if validated.confidence < 0.85:
        create_review_task(workflow_run_id, validated)
    else:
        save_artifact(f"03_Extracted_Data/", validated)
...
### Inbound Webhook
```
POST /api/v1/workflows/03_Extracted_Data/run
...
POST {subscriber_url}
{
  ...
  "artifact_path": "03_Extracted_Data/REQ-2026-0142_CAN-88421_output.json"
}
```
...
- **Library:** `RecruitmentOps/03_Extracted_Data/`
...
Parse inbound applications via dedicated alias `careers+03_Extracted_Data@{{org.email_domain}}`.
