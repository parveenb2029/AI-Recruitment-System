# Job Description Parsing and Management — Automation Implementation Guide
...
| Component | Responsibility in JD Parser |
|-----------|--------------------------------|
...
| **SharePoint** | Primary document library for `01_Job_Descriptions/` |
...
```
1. Open 09_Prompt_Library/ and 01_Job_Descriptions/Prompt.md in Cursor.
...
```python
# Pseudocode — production worker for WF-01
def process_item(workflow_run_id, payload):
    ...
    validated = validate_schema(result, schema="WF-01_output")
    if validated.confidence < 0.85:
        create_review_task(workflow_run_id, validated)
    else:
        save_artifact(f"01_Job_Descriptions/", validated)
...
### Inbound Webhook
```
POST /api/v1/workflows/01_Job_Descriptions/run
...
POST {subscriber_url}
{
  ...
  "artifact_path": "01_Job_Descriptions/REQ-2026-0142_CAN-88421_output.json"
}
```
...
- **Library:** `RecruitmentOps/01_Job_Descriptions/`
...
Parse inbound applications via dedicated alias `careers+01_Job_Descriptions@{{org.email_domain}}`.
