# Interview Feedback Processing — Automation Implementation Guide
...
| Component | Responsibility in Feedback Processor |
|-----------|--------------------------------|
...
| **SharePoint** | Primary document library for `07_Interview_Feedback/` |
...
```
1. Open 09_Prompt_Library/ and 07_Interview_Feedback/Prompt.md in Cursor.
...
```python
# Pseudocode — production worker for WF-07
def process_item(workflow_run_id, payload):
    ...
    validated = validate_schema(result, schema="WF-07_output")
    if validated.confidence < 0.85:
        create_review_task(workflow_run_id, validated)
    else:
        save_artifact(f"07_Interview_Feedback/", validated)
...
### Inbound Webhook
```
POST /api/v1/workflows/07_Interview_Feedback/run
...
POST {subscriber_url}
{
  ...
  "artifact_path": "07_Interview_Feedback/REQ-2026-0142_CAN-88421_output.json"
}
```
...
- **Library:** `RecruitmentOps/07_Interview_Feedback/`
...
Parse inbound applications via dedicated alias `careers+07_Interview_Feedback@{{org.email_domain}}`.
