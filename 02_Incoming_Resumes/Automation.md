# Incoming Resume Intake — Automation Implementation Guide
...
| Component | Responsibility in Resume Intake |
|-----------|--------------------------------|
...
| **SharePoint** | Primary document library for `02_Incoming_Resumes/` |
...
```
1. Open 09_Prompt_Library/ and 02_Incoming_Resumes/Prompt.md in Cursor.
...
```python
# Pseudocode — production worker for WF-02
def process_item(workflow_run_id, payload):
    ...
    validated = validate_schema(result, schema="WF-02_output")
    if validated.confidence < 0.85:
        create_review_task(workflow_run_id, validated)
    else:
        save_artifact(f"02_Incoming_Resumes/", validated)
...
### Inbound Webhook
```
POST /api/v1/workflows/02_Incoming_Resumes/run
...
POST {subscriber_url}
{
  ...
  "artifact_path": "02_Incoming_Resumes/REQ-2026-0142_CAN-88421_output.json"
}
```
...
- **Library:** `RecruitmentOps/02_Incoming_Resumes/`
...
Parse inbound applications via dedicated alias `careers+02_Incoming_Resumes@{{org.email_domain}}`.
