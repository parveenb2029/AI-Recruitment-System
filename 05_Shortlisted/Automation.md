# Candidate Shortlisting — Automation Implementation Guide
...
| Component | Responsibility in Shortlist |
|-----------|--------------------------------|
...
| **SharePoint** | Primary document library for `05_Shortlisted/` |
...
```
1. Open 09_Prompt_Library/ and 05_Shortlisted/Prompt.md in Cursor.
...
```python
# Pseudocode — production worker for WF-05
def process_item(workflow_run_id, payload):
    ...
    validated = validate_schema(result, schema="WF-05_output")
    if validated.confidence < 0.85:
        create_review_task(workflow_run_id, validated)
    else:
        save_artifact(f"05_Shortlisted/", validated)
...
### Inbound Webhook
```
POST /api/v1/workflows/05_Shortlisted/run
...
POST {subscriber_url}
{
  ...
  "artifact_path": "05_Shortlisted/REQ-2026-0142_CAN-88421_output.json"
}
```
...
- **Library:** `RecruitmentOps/05_Shortlisted/`
...
Parse inbound applications via dedicated alias `careers+05_Shortlisted@{{org.email_domain}}`.
