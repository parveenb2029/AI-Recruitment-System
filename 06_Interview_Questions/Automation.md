# Interview Question Generation — Automation Implementation Guide
...
| Component | Responsibility in Interview Gen |
|-----------|--------------------------------|
...
| **SharePoint** | Primary document library for `06_Interview_Questions/` |
...
```
1. Open 09_Prompt_Library/ and 06_Interview_Questions/Prompt.md in Cursor.
...
```python
# Pseudocode — production worker for WF-06
def process_item(workflow_run_id, payload):
    ...
    validated = validate_schema(result, schema="WF-06_output")
    if validated.confidence < 0.85:
        create_review_task(workflow_run_id, validated)
    else:
        save_artifact(f"06_Interview_Questions/", validated)
...
### Inbound Webhook
```
POST /api/v1/workflows/06_Interview_Questions/run
...
POST {subscriber_url}
{
  ...
  "artifact_path": "06_Interview_Questions/REQ-2026-0142_CAN-88421_output.json"
}
```
...
- **Library:** `RecruitmentOps/06_Interview_Questions/`
